...
public function __construct ($host='',$port=80,$path='/') {
   	$this->websocket_open($host,$port,$path);
   	}
...
function websocket_open($host='',$port=80,$path='/',$headers='',&$error_string='',$timeout=10,$ssl=false){
  ...
  $this->sp = stream_socket_client($address, $errno, $errstr, $timeout);
  ...
  // Set timeouts
  stream_set_timeout($this->sp,$timeout);
...
\*============================================================================*/
function write($data,$final=true){
	$sp = $this->sp;
...
function close(){
	$sp = $this->sp;
	stream_socket_shutdown ( $sp , STREAM_SHUT_WR);
}

static function daemonSend($data){
	$path = env('WEBSITE_WEBSOCKET_NAME').":".env('WEBSITE_WEBSOCKET_APIKEY');

	SocketClient::sendOncePath(env('DAEMON_WEBSOCKET_ADDRESS'),
		$path,
		env('DAEMON_WEBSOCKET_PORT'),
		$data
	);
}

static function daemonSendMultiple($data){
	$path = env('WEBSITE_WEBSOCKET_NAME').":".env('WEBSITE_WEBSOCKET_APIKEY');

	SocketClient::sendMultiplePath(env('DAEMON_WEBSOCKET_ADDRESS'),
		$path,
		env('DAEMON_WEBSOCKET_PORT'),
		$data
	);
...
static function sendOnce($host,$port,$data){
	$s = new SocketClient($host,$port);
	$s->write($data);
	usleep( 150000 );
	$s->close();
}

static function sendOncePath($host,$path,$port,$data){
	$s = new SocketClient($host,$port,$path);
	$s->write($data);
	usleep( 150000 );
	$s->close();
}
static function sendMultiplePath($host,$path,$port,$data){
	$s = new SocketClient($host,$port,$path);
	foreach($data as $d){
		$s->write($d);
	}
